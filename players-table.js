var TableDatatablesEditable = function () {
    ...
    var handleTable = function () {
        ...
        function editRow(oTable, nRow) {
            ...
            jqTds[0].innerHTML = '<input id="number" type="text" class="form-control input-small is_required" value="' + aData[0] + '">';
			jqTds[1].innerHTML = '<select id="position_list" class="form-control input-small teams"><option disabled selected value="0">Выбрать</option><option value="1">Вратарь</option><option value="2">Защитник</option><option value="3">Универсал</option><option value="4">Нападающий</option></select>';
			jqTds[2].innerHTML = '<input type="text" class="form-control input-small is_required" value="' + aData[2] + '">';
			jqTds[3].innerHTML = '<input type="text" class="form-control input-small is_required" value="' + aData[3] + '">';
        ...
        function saveRow(oTable, nRow) {
			...
			var position = $("#position_list :selected").text();
            oTable.fnUpdate(jqInputs[0].value, nRow, 0, false);
			oTable.fnUpdate(position, nRow, 1, false);
        ...
        {
           var check = $('input.is_required',nRow).filter(function() { return $(this).val() == ""; });
           var errors = false;
           $('.help-block',nRow).remove();
            ...
            if(!errors)
            {
                ...
                if (!$.isNumeric($('#number', nRow).val()))
                {
                    $('#number', nRow).css('border-color', '#e73d4a')
                            .after('<span class="help-block">Поле должно быть числовым!</span>');
                    errors = true;
                }
            }
       return errors;
        ...
        table.on('click', '.edit', function (e) {
            ...
            if (nEditing !== null && nEditing != nRow) {
                ...
                {
                var position = saveRow(oTable, nEditing);
                var jqInputs = $('input', nRow);
                var aData = oTable.fnGetData(nRow);
                var formData = {
                    number: aData[0],
